...
void main() {
	...
	vec3 ray_dir = normalize((view * vec4(p, camera.fov, 0.0)).xyz);
	
	float t = -ray_origin.z / ray_dir.z;
	vec2 c = (ray_origin + t * ray_dir).xy;
	
	float halfX = 0.5 / (resolution.x * zoom);
	float halfY = 0.5 / (resolution.y * zoom);
	
	vec4 colors[4];
	colors[0] = getColor(mandelbrot(c));
	//colors[1] = getColor(mandelbrot(c + vec2(0.0, halfY)));
	//colors[2] = getColor(mandelbrot(c + vec2(halfX, 0.0)));
	//colors[3] = getColor(mandelbrot(c + vec2(halfX, halfY)));
	
	FragColor = colors[0]; //(colors[0] + colors[1] + colors[2] + colors[3]) / 4.0;
}